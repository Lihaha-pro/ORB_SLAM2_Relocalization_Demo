...
namespace ORB_SLAM2
{
    ...
    vector<KeyFrame *> KeyFrameDatabase::DetectRelocalizationCandidates(Frame *F)
    {
        ...
        {
            unique_lock<mutex> lock(mMutex);
            cout << "该关键帧观测到单词数：" << F->mBowVec.size() << endl;
            for (DBoW2::BowVector::const_iterator vit = F->mBowVec.begin(), vend = F->mBowVec.end(); vit != vend; vit++)
            {
                list<KeyFrame *> &lKFs = mvInvertedFile[vit->first]; ///观测到某单词的关键帧的list
                // cout << "观测到某单词的关键帧：" << lKFs.size() << endl;
        ...
        cout << "有共视单词的关键帧数量为：" << lKFsSharingWords.size() << endl;
        if (lKFsSharingWords.empty())
        {
        ...
        int minCommonWords = maxCommonWords * 0.2f; ///本来为0.8
        ...
        cout << "共视单词数阈值筛选后关键帧数量为：" << lScoreAndMatch.size() << endl;
